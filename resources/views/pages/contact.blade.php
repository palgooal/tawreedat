...
@section('content')
    ...
    <section class="relative flex min-h-[320px] items-center overflow-hidden bg-gov-950 py-14 text-white sm:py-16">
        ...
        <div class="relative mx-auto w-full max-w-[1500px] px-4 sm:px-6 lg:px-8">
            <div class="max-w-3xl">
                ...
                <p class="mt-4 max-w-3xl text-sm leading-8 text-slate-100 sm:text-base">يسعدنا استقبال طلبات الشركات
                    والموردين والجهات الباحثة عن موردين، وتوجيه كل طلب إلى المسار المناسب داخل منصة توريد.</p>
            </div>
        </div>
    </section>
    ...
    <section class="py-14 sm:py-16">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mb-8 flex flex-wrap gap-6 text-sm font-semibold text-gov-900">
                <span class="inline-flex items-center gap-2"><span class="text-gold-600" aria-hidden="true">✓</span>رد خلال
                    يوم عمل</span>
                <span class="inline-flex items-center gap-2"><span class="text-gold-600" aria-hidden="true">✓</span>دعم
                    للشركات والموردين</span>
                <span class="inline-flex items-center gap-2"><span class="text-gold-600" aria-hidden="true">✓</span>قنوات
                    تواصل مباشرة</span>
            </div>
                ...
                <div>
                    <form method="POST" action="{{ route('contact.store') }}"
                        ...
                        <div>
                            ...
                            <p class="mt-3 text-sm leading-7 text-slate-600">أرسل التفاصيل الأساسية وسيتولى الفريق المختص
                                مراجعة الطلب وتوجيهه.</p>
                            <p class="mt-2 text-xs font-semibold text-slate-600">الحقول المميزة بعلامة <span
                                    class="text-gold-600">*</span> مطلوبة.</p>
                        </div>

                        <div class="mt-8 grid gap-5 sm:grid-cols-2">
                            <div>
                                <label for="full-name" class="text-xs font-bold text-gov-950">الاسم الكامل <span
                                        class="text-gold-600" aria-hidden="true">*</span></label>
                                <input id="full-name" name="name" value="{{ old('name') }}" autocomplete="name"
                            ...
                            <div>
                                <label for="email" class="text-xs font-bold text-gov-950">البريد الإلكتروني <span
                                        class="text-gold-600" aria-hidden="true">*</span></label>
                                ...
                                @error('email')
                                    <p id="email-error" class="mt-1.5 text-xs font-semibold text-red-600">{{ $message }}
                                    </p>
                                @enderror
                            </div>
                            <div>
                                ...
                                @error('phone')
                                    <p id="phone-error" class="mt-1.5 text-xs font-semibold text-red-600">{{ $message }}
                                    </p>
                                @enderror
                            </div>
                            <div>
                                <label for="request-type" class="text-xs font-bold text-gov-950">نوع الطلب <span
                                        class="text-gold-600" aria-hidden="true">*</span></label>
                                <select id="request-type" name="inquiry_type" required
                                    aria-invalid="@error('inquiry_type') true @else false @enderror"
                                    @error('inquiry_type') aria-describedby="inquiry-type-error" @enderror
                                ...
                                @error('inquiry_type')
                                    <p id="inquiry-type-error" class="mt-1.5 text-xs font-semibold text-red-600">
                                        {{ $message }}</p>
                                @enderror
                            </div>
                            <div class="sm:col-span-2">
                                <label for="organization" class="text-xs font-bold text-gov-950">اسم الشركة /
                                    الجهة</label>
                                <input id="organization" name="company" value="{{ old('company') }}"
                                    autocomplete="organization"
                                    aria-invalid="@error('company') true @else false @enderror"
                                ...
                                @error('company')
                                    <p id="company-error" class="mt-1.5 text-xs font-semibold text-red-600">
                                        {{ $message }}</p>
                                @enderror
                            </div>
                            <div class="sm:col-span-2">
                                <label for="message" class="text-xs font-bold text-gov-950">الرسالة <span
                                        class="text-gold-600" aria-hidden="true">*</span></label>
                                <textarea id="message" name="message" required aria-invalid="@error('message') true @else false @enderror"
                                ...
                                @error('message')
                                    <p id="message-error" class="mt-1.5 text-xs font-semibold text-red-600">
                                        {{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <button type="submit"
                                class="inline-flex h-12 items-center justify-center rounded-xl bg-gov-800 px-6 text-sm font-bold text-white transition hover:bg-gov-700">إرسال
                                الطلب</button>
    ...
    <section class="pb-16">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div
                class="flex flex-col gap-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-8">
                <div>
                    <h2 class="text-2xl font-bold text-gov-950">هل ترغب في ظهور شركتك ضمن توريد؟</h2>
                    <p class="mt-2 text-sm text-slate-600">ابدأ بإنشاء حضور مهني يساعد الجهات والمقاولين على الوصول إليك
                        بوضوح.</p>
                </div>
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('company-registration.create') }}"
                        class="inline-flex h-11 items-center justify-center rounded-xl bg-gov-800 px-5 text-sm font-bold text-white transition hover:bg-gov-700">سجل
                        شركتك</a>
                    <a href="{{ route('companies.index') }}"
                        class="inline-flex h-11 items-center justify-center rounded-xl border border-slate-200 px-5 text-sm font-bold text-gov-800 transition hover:border-gold-300">استعرض
                        الشركات</a>
                </div>
            </div>
        </div>
